...
plot(ts(l[1:11], s = 0), xlim = c(0, 9.6), ylim = c(-1, 1), ylab = "ell")
abline(0, 0, lty = 3)
...
par(mfrow = c(3, 1))
set.seed(4)
y <- simulate.ets(mod, 2000, initstate = 10, bootstrap = FALSE)
plot(y, )
y <- simulate.ets(mod, 2000, initstate = 10, bootstrap = FALSE)
plot(y)
y <- simulate.ets(mod, 2000, initstate = 10, bootstrap = FALSE)
...
plot(
  ts(c(M3$N0193$x, M3$N0193$xx), s = 1947),
  xlab = "Year",
  ylab = "New freight cars"
)
...
plot(
  ...
  xlim = c(ll, ul),
  ylim = c(ll, ul),
  xlab = "MASE of model 1",
  ylab = "MASE of model 3",
  main = ""
)
