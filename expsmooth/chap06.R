...
plot(
  frexport,
  xlim = c(1, 10.8),
  ylim = range(frexport, z),
  ylab = "Quarterly sales (thousands of francs)",
  xlab = "Year"
)
for (i in 1:20) lines(z[, i], col = "gray")
...
plot(
  density(z[16, ], bw = "SJ"),
  main = "Quarterly sales distribution: 16 steps ahead",
  xlab = "Sales (thousands of francs)"
)
...
plot(
  ltd.den,
  main = "Lead time demand distribution: 3-steps ahead",
  xlab = "Sales (millions of francs)"
)
text(median(ltd) / 1e3, 0.2, "90% Prediction Interval")
lines(
  quantile(ltd / 1e3, type = 8, prob = c(.05, .95)),
  c(0, 0),
  lwd = 3,
  col = 1
)
...
for (j in 1:24) {
  ...
  k <- as.integer((j - 1) / 4)
  vv[j] <- ((1 + fit$sigma2) *
    (1 + fit$par["gamma"]^2 * fit$sigma2)^k *
    theta[j] -
    mu[j]^2) *
    zn[idx]^2
  if (j < 24) {
    theta[j + 1] <- mu[j + 1]^2 +
      fit$sigma2 *
        sum(
          (fit$par["alpha"] + (1:j) * fit$par["beta"])^2 * theta[j + 1 - (1:j)]
        )
  }
}
...
plot.ts(
  sqrt(v[1:12]),
  ylab = "Forecast standard deviation",
  xlab = "Forecast horizon"
)
lines(sqrt(vv[1:12]), lty = 2)
legend(
  "topleft",
  legend = c("Exact", "Small sigma approximation"),
  lty = c(1, 2)
)

plot.ts(
  c(frexport, rep(NA, 12)),
  ylim = c(300, 1500),
  ylab = "",
  xlab = "Quarter"
)
...
legend(
  "topleft",
  legend = c("Percentile-based interval", "Variance-based interval"),
  lty = c(2, 1)
)
...
var1 <- (elln^2 + sigma^2) *
  h +
  sigma^2 * alpha * ((1 + 0.5 * alpha) * h + alpha * h^3 / 3)
...
plot(
  ...
  var1,
  type = "l",
  main = "Lead-time demand variance",
  ylab = "Variance",
  xlab = "Forecast horizon"
)
lines(h, var2, lty = 2)
