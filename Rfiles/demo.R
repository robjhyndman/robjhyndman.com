...
plot(
  life.expectancy(france.forecast),
  ylab = "Forecast life expectancy",
  xlab = "Year"
)
...
france.ISFE <- cbind(
  france.e1$int.error[, 2],
  france.e2$int.error[, 2],
  france.e3$int.error[, 2],
  france.e4$int.error[, 2],
  france.e5$int.error[, 2]
)

plot(
  ts(sqrt(france.ISFE), s = 1978),
  plot.type = "single",
  col = 2:6,
  xlab = "h",
  ylab = "Root ISFE",
  main = "Root ISFE for different K",
  xlim = 1977 + c(1, 22),
  lwd = 2,
  cex = 0.7
)
for (i in 1:5) text(1977 + 21, sqrt(france.ISFE[20, i]), paste("K=", i))
...
france.compare <- sqrt(cbind(
  france.e2$int.error[, 2],
  france.e.LC$int.error[, 2],
  france.e.BMS$int.error[, 2]
))

plot(
  ts(france.compare, s = 1978),
  plot.type = "single",
  col = 2:6,
  xlab = "h",
  ylab = "Root ISFE",
  main = "Root ISFE for different methods",
  lwd = 2
)
