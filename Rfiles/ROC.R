...
make.K <- function(x, h, xgrid, ngrid = 200, lo, hi) {
  ...
  if (missing(xgrid)) {
    if (missing(lo)) lo <- min(x) - 3 * h
    if (missing(hi)) hi <- max(x) + 3 * h
...
cdf.est <- function(x, h = h.lloyd2(x), xgrid) {
  if (missing(xgrid)) xgrid <- seq(min(x) - 3 * h, max(x) + 3 * h, l = 200)
  n <- length(xgrid)
...
pdf.est <- function(x, h = bw.SJ(x), xgrid) {
  ...
  return(list(
    x = xgrid,
    y = density(x, bw = h, from = xgrid[1], to = xgrid[n], n = n)$y
  ))
}
...
reduce <- function(
  x = stop("Argument is missing"),
  y = stop("Argument is missing")
) {
  Y <- y[order(x)]
  X <- sort(x)
...
hx.norm <- function(mx, sx, my, sy, n) {
  ...
  f1 <- 4 *
    ...
    sx^3 *
    sum1^2.5 /
    (sqrt(sum2) * (sy^4 + sy^2 * sx^2 + 2 * sx^2 * sum3))
...
pluginbands <- function(x, y) {
  ...
  kappa2 <- 1
  I1 <- abs(
    integrate(intf9, rmin, rmax, mx = my, sx = sy, my = mx, sy = sx)$value
  )
  I2 <- abs(
    integrate(intf3, rmin, rmax, mx = my, sx = sy, my = mx, sy = sx)$value
  )
  I3 <- abs(
    integrate(intf2, rmin, rmax, mx = my, sx = sy, my = mx, sy = sx)$value
  )
  H <- n^(-0.4) *
    ((2 * R * lambda1 * I1 + lambda2 * I2) / (2 * kappa2^2 * I3^2))^(0.2)

  I4 <- abs(
    integrate(intf5, rmin, rmax, mx = mx, sx = sx, my = my, sy = sy)$value
  )
  I5 <- abs(
    integrate(intf6, rmin, rmax, mx = mx, sx = sx, my = my, sy = sy)$value
  )
  J <- 64 *
    abs(integrate(intf10, rmin, rmax, mx = mx, sx = sx, my = my, sy = sy)$value)
  rho <- h.lloyd(y) / h.lloyd(x)
  H2 <- (3 *
    J /
    (m * n * sqrt(2 * pi) * (1 + 2 * rho^2)^1.5 * (2 * rho^2 * I4 + I5)^2))^(1 /
    7)
  H1 <- rho * H2
...
crossval.int <- function(H, H1, H2, x, y, diag = 1) {
  ...
  g2 <- numeric(m)
  for (i in 1:m) {
    tmp <- matrix(y.K$K2[i, ], nrow = n, ncol = 1) %*%
      matrix(y.K$K2[i, ], nrow = 1, ncol = n)
    g2[i] <- 2 * sum(tmp[lower.tri(tmp, diag = F)]) / (n * (n - 1) * H^2)
  }
  for (i in 1:n) {
    tmp <- matrix(x.K$K3[i, ], nrow = m, ncol = 1) %*%
      matrix(x.K$K3[i, ], nrow = 1, ncol = m)
    if (diag == 1) {
      f12[i] <- (2 * sum(tmp[lower.tri(tmp, diag = T)]) - sum(diag(tmp))) /
        (m * m * H1^4)
    } else if (diag == 0) {
      f12[i] <- 2 *
        sum(tmp[lower.tri(tmp, diag = F)]) /
        (m * (m - 1) * H1^4)
    } else {
      f12[i] <- (2 *
        sum(tmp[lower.tri(tmp, diag = T)]) -
        1.5 * sum(diag(tmp))) /
        (m * (m - 0.5) * H1^4)
    }
...
ROC.norm.mix <- function(
  ...
  py2 <- py2 / sump
  x <- seq(
    min(mx1, mx2) - 5 * max(sx1, sx2),
    max(mx1, mx2) + 5 * max(sx1, sx2),
    l = nx
  )
  y <- seq(
    min(my1, my2) - 5 * max(sy1, sy2),
    max(my1, my2) + 5 * max(sy1, sy2),
    l = nx
  )
...
ASMCONF.est <- function(x, y, alpha = 0.05, ngrid = 512, ...) {
  ...
  sig2 <- sqrt(
    1 / n * GFinv * (1 - GFinv) + 1 / m * tt * (1 - tt) * (gFinv / fFinv)^2
  )
