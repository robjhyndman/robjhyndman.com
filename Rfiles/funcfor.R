...
plot(
  fr.mort,
  series = "male",
  years = c(1899, 1918, 1957, 1977, 1997),
  type = "p",
  cex = 0.6,
  col = 1,
  pch = c(1, 15, 22, 2, 16),
...
legend(
  5,
  1.5,
  legend = paste(c(1899, 1918, 1957, 1977, 1997)),
  col = 1,
  pch = c(1, 15, 22, 2, 16),
  cex = 0.8
)
...
i <- c(1901, 1973)
plot(fr.mort, years = i, type = "p", pch = 1)
lines(france.sm, years = i)
...
ausfert$rate$female <- rbind(
  rep(0.005, 80),
  ausfert$rate$female,
  rep(0.005, 80)
)
ausfert$pop$female <- rbind(
  ausfert$pop$female[1, ],
  ausfert$pop$female,
  ausfert$pop$female[7, ]
)
rownames(ausfert$rate$female) <- rownames(ausfert$pop$female) <- c(
  "<15",
  "15-19",
  "20-24",
  "25-29",
...
text(
  rep(2004, 7) - c(0, 0, 8, 8, 0, 0, 0),
  log(aus.fert$rate[[1]])[, 80] + c(0, .1, .4, -.2, -.1, 0, 0),
  rownames(aus.fert$rate$female),
  cex = 0.7
)
...
plot(fert.sm, transform = TRUE)
points(ausfert, transform = TRUE)
