...
lagphase <- function(data, knots = NULL, order = 1, gam = FALSE, zeros = TRUE) {
  ...
  if (is.null(order)) {
    if (!is.null(knots)) {
      stop(
        "Not implemented. If you specify the knots, you need to specify the order."
      )
    }
    fit1 <- lagphase(data, order = 1)
    fit3 <- lagphase(data, order = 3)
    bestfit <- fit1
    if (AICc(fit3) < AICc(bestfit)) bestfit <- fit3
    return(bestfit)
  }
  ...
  bestfit <- fit4
  if (fit3$value < bestfit$value) bestfit <- fit3
  if (fit2$value < bestfit$value) bestfit <- fit2
  if (fit1$value < bestfit$value) bestfit <- fit1
...
lagphase.knots <- function(knots, data, order) {
  ...
  for (i in 1:length(knots)) x[, i] <- pmax((data$year - knots[i])^order, 0)
  ...
  if (length(fit$knots) > 0) {
    if (fit$coef[2] > 0) fit$lengthlag <- fit$knots[1] - data$year[1]
  }
...
plot.lagphase <- function(
  ...
  if (is.null(ylim)) ylim <- range(lo, up)
  ...
  polygon(
    c(fit$data$year[j], rev(fit$data$year[j])),
    c(lo, rev(up)),
    border = FALSE,
    col = "gray"
  )
  lines(fit$data$year[j], adjfits)
  if (!is.null(fit$knots)) abline(v = fit$knots[1], col = "gray")
...
freqplot <- function(
  ...
  j <- (data$specimens > 0)
  if (!is.null(fit1)) lines(data$year[j], fitted(fit1), col = cols[1])
  if (!is.null(fit2)) lines(data$year[j], fitted(fit2), col = cols[2])
  if (!is.null(fit3)) lines(data$year[j], fitted(fit3), col = cols[3])
  if (!is.null(fit4)) lines(data$year[j], fitted(fit4), col = cols[4])
}
