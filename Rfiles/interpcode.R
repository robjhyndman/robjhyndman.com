cm.spline <- function(
  ...
  if (is.na(method)) stop("spline: invalid interpolation method")
  dx <- diff(x)
  if (any(dx < 0)) {
    o <- order(x)
    x <- x[o]
  ...
  if (any(diff(y) < 0)) stop("Data are not monotonic")
...
cm.splinefun <- function(x, y = NULL, method = "fmm", gulim = 0) {
  ...
  n <- length(x)
  method <- match(method, c("periodic", "natural", "fmm"))
  if (is.na(method)) stop("splinefun: invalid interpolation method")
  if (any(diff(x) < 0)) {
    z <- order(x)
    x <- x[z]
    y <- y[z]
...
spl.coef.conv <- function(z) {
  ...
  c1 <- (3 * (y0[n - 1] - y1[n - 1]) + (b0[n - 1] + 2 * b1[n - 1]) * h[n - 1]) /
    h[n - 1]^2
