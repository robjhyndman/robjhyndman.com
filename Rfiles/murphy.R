murphydiagram <- function(
  ...
  n <- length(y)
  if (length(f1) != n) stop("Forecasts should be of same length as data")
  ...
  if (xaxis == "time") {
    ...
    if (type == "diff") {
      ...
      xidx <- time(sdiff)
      polygon(
        c(-1e20, -1e20, 1e20, 1e20),
        c(lower, upper, upper, lower),
        col = gray(0.8),
        border = FALSE
      )
      if (tspx) {
