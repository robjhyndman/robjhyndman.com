...
plot(
  ts(sa[, "temp1"], start = c(2000, 7 * 48), freq = seasondays * 48),
  xlab = "Year",
  ylab = "degrees C"
)
plot(
  ts(sa[, "temp2"], start = c(2000, 7 * 48), freq = seasondays * 48),
  xlab = "Year",
  ylab = "degrees C"
)
...
plot.ts(
  ts(demand$demand[, sample(1:10, 4)], freq = 48, start = 0),
  xlab = "Days",
...
boxplot(
  demand$annmax,
  main = "Simulated seasonal maximums",
  xlab = "GW",
  horizontal = TRUE
)
rug(demand$annmax)
