...
# created with whatever fields the bib entry provides.
...
# (a couple of entries do) back to "First Last".
...
build_chapter_details <- function(e) {
  ...
  if (is.null(booktitle)) {
    return(NULL)
  }
  booktitle <- str_replace(unescape_latex(booktitle), "\\.$", "")
  out <- paste0("<em>", booktitle, "</em>")
  if (!is.null(e$editor)) {
    out <- paste0(out, ", ", format_editors(e$editor))
  }
  if (!is.null(e$chapter)) {
    out <- paste0(out, ", Chapter ", e$chapter)
  }
  if (!is.null(e$pages)) {
    label <- if (is_page_range(e$pages)) "pp." else "p."
    out <- paste0(out, ", ", label, " ", format_pages(e$pages))
  }
  if (!is.null(e$publisher)) {
    out <- paste0(out, ". ", unescape_latex(e$publisher))
  }
  if (!is.null(e$address)) {
    out <- paste0(out, ", ", unescape_latex(e$address))
  }
  out
}

build_details <- function(e) {
  if (e$.type %in% c("article", "review")) {
    build_journal_details(e)
  } else if (e$.type %in% c("inbook", "incollection", "inproceedings")) {
    build_chapter_details(e)
  } else {
    NULL
  }
}
...
# existing "field: value" entry, so it can be replaced in place. A long
# value (quoted or not) folds onto indented continuation lines in YAML
# block-mapping style, so indentation alone -- not the presence of quotes --
# is what marks a line as part of the same entry.
...
# bib-derived field the entry provides, given the lines already read for the
# bibkey->path mapping (so the file isn't read from disk a second time).
# Returns TRUE if the file changed.
sync_file <- function(path, lines, e) {
  dashes <- which(lines == "---")
  if (length(dashes) < 2) {
    return(FALSE)
  }
  ...
  for (field in names(values)) {
    ...
    if (is.null(span)) {
      ...
      !(span$start == span$end && lines[span$start] == generated_line)
    ) {
      lines <- append(
...
create_file <- function(key, e) {
  ...
  if (!is.null(values$author)) {
    fm <- c(fm, paste0("author: ", values$author))
  }
  if (!is.null(values$title)) {
    fm <- c(fm, paste0("title: ", values$title))
  }
  if (!is.null(e$year)) {
    fm <- c(fm, paste0("date: ", str_trim(e$year), "-01-01"))
  }
  fm <- c(fm, paste0("categories: ", category_for_type(e$.type)))
  if (!is.null(values$details)) {
    fm <- c(fm, paste0("details: ", values$details))
  }
  if (!is.null(values$doi)) {
    fm <- c(fm, paste0("doi: ", values$doi))
  }
  fm <- c(fm, "---", "")

  writeLines(fm, path, useBytes = TRUE)
  # category_for_type("article") is a guess (that bib type also covers this
  # site's "Editorials" and "Miscellaneous" categories) -- surface that so
  # it doesn't read as confidently correct in the run's report.
  list(path = path, category_guessed = identical(e$.type, "article"))
}
...
files <- unique(c(
  ...
  Sys.glob("publications/*/index.qmd"),
  Sys.glob("publications/*.md"),
  Sys.glob("publications/*.qmd")
))
files <- files[!grepl("_metadata", files)]

# Map every publication file's bibkey -> path (keeping the lines already
# read, so sync_file doesn't have to read the file from disk again), so each
# bib entry can find its one matching file (or learn it needs to be
# created). Flags a bibkey claimed by more than one file, and a file whose
# bibkey isn't in the bib at all, as warnings -- both indicate the 1:1
# mapping is broken and need a person to sort out, so this script doesn't
# try to guess which file wins.
key_to_path <- list()
key_to_lines <- list()
...
for (f in files) {
  ...
  key_to_lines[[bibkey]] <- lines
  if (is.null(bib[[bibkey]])) {
    problems <- c(
      problems,
      sprintf("%s has bibkey '%s', which is not in rjhpubs.bib", f, bibkey)
    )
  }
}
...
for (key in names(bib)) {
  ...
  if (is.null(path)) {
    created[[length(created) + 1]] <- create_file(key, e)
  } else if (sync_file(path, key_to_lines[[key]], e)) {
...
if (length(created) > 0) {
  ...
  for (c in created) {
    note <- if (c$category_guessed) {
      " (category guessed as Articles -- verify)"
    } else {
      ""
    }
    cat(" - ", c$path, note, "\n", sep = "")
...
if (length(problems) == 0 && length(created) == 0 && length(updated) == 0) {
  cat(
    "sync_bib_fields: nothing to sync --",
    length(bib) - length(EXCLUDE_KEYS),
    "bib entries already match their publication.\n"
  )
}
