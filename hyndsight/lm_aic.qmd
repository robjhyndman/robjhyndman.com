...
$$
Since we don't know $\sigma^2$, we estimate it using the residual sum of squared errors (SSE) divided by $n$, giving
\begin{align*}
\text{AIC} & = 2k +n\log(2\pi) + n\log\left(\frac{\text{SSE}}{n}\right) + n \\
& = 2k + n\log\left(\frac{\text{SSE}}{n}\right) + C
\end{align*}
