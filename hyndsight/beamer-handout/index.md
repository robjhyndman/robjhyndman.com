...
I'm about to head off on a speaking tour to Europe (more on that in another post) and one of my hosts has asked for my powerpoint slides so they can print them.
They have made two false assumptions: (1) that I use powerpoint; (2) that my slides are static so they can be printed.
...
Then I produced a pdf file with several slides per page.
<!-- more -->
...
When the file is compiled, each slide should now be printable.
But I want several slides on each page.
So I then use the **[`pdfnup`](https://github.com/pdfjam/pdfjam)** command (sorry Windows users, you need a real computer to do this):
