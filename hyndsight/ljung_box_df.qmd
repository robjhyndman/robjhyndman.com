...
As a guess, perhaps the seasonal lags aren't having an effect with $\ell=10$. We can test what happens for larger $\ell$ by setting $\ell=24$ (covering two years), and repeating the exercise.
...
The test with $K=1$ looks the most uniform, with the size of the test closest to the nominal values. So $K=p+q$ seems to be correct here.
...
Here there are 3 smoothing parameters, and 13 initial states to estimate. So I was expecting $K=3$ to do best, but it is the worse. Instead, $K=0$ is the best.
