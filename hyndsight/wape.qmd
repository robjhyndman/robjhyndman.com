...
$$
\text{MASE} = \frac{\sum_{t} |y_t - \hat{y}_t|}{\sum_{t=1}^T |y_t - \hat{y}_{t-m}|}
$$
where $m=1$ for non-seasonal data, and $m$ is the seasonal period for seasonal data, and the sum in the numerator is over the *test* set, while the sum in the denominator is over the *training* set. Because the denominator is defined on the *training* data, not the *test* data, it avoids the above problems with the WAPE:
...
$$
\text{RMSSE} = \sqrt{\frac{\sum_{t} (y_t - \hat{y}_t)^2}{\sum_{t=1}^T (y_t - \hat{y}_{t-m})^2}},
$$
where again the sum in the numerator is over the *test* set, and the sum in the denominator is over the *training* set. This has all the advantages (and disadvantages) of the MASE, but optimising it leads to the mean forecast rather than the median. It also aligns better with how models are estimated. Almost all models are estimated by minimising the sum of squared errors, so it makes sense to evaluate them using squared errors as well.
