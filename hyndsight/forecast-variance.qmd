...
description: There is sometimes confusion between "forecast variance" and "forecast error variance". In fact, they are the same thing, assuming we have a good model and ignoring estimation error.
---
...
I've also ignored model error. That is, I have assumed our model is at least approximately correct, so that the estimated mean and variance from the model are reasonable estimates of the true mean and variance. If the model is wrong, then the forecast error variance could be much larger than the (true) forecast variance.
