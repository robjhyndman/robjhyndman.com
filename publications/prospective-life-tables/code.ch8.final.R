...
plot(usa1950, years=2003, series='male',type="p", pch=1, col="gray", main="")
lines(smus1950, years=2003, series='male')

# CDE method
Ext <- usa1950$pop$male
Dxt <- usa1950$rate$male * Ext
fitBIC <- Mort2Dsmooth(x=usa1950$age, y=usa1950$year, Z=Dxt, offset=log(Ext))

par(mfrow=c(1,2))
plot(fitBIC$x, log(usa1950$rate$male[,"2003"]), xlab="Age", ylab="Log death rate",col="gray",  main="USA: male death rates 2003")
...
par(mfrow=c(1,2))
persp(age, year, log(usa1950$rate$male[seq(1,111,by=3), seq(1,62,by=3)]),theta=-30,
	main="Observed death rates", col=grey(.93), shade=TRUE, xlab="Age", ylab="", 
      zlab="Log death rate", ticktype="detailed",cex.axis=0.6,cex.lab=0.6)
persp(age, year, predict(fitBIC)[seq(1,111,by=3),seq(1,62,by=3)], theta=-30,
	main="Smoothed death rates", col=grey(.93), shade=TRUE, xlab="Age",  ylab="",
      zlab="Log death rate", ticktype="detailed",cex.axis=0.6,cex.lab=0.6)
...
whiA <- c(66,86)
plot(usa1950, series="male", age=whiA-1, plot.type="time", 
     xlim=c(1950,2030), ylim=c(-6.2,-1), xlab="Year", 
     main="USA: forecast male death rates using 2-dimensional CDE method", col=c(1,2))
matlines(forecastyears, t(CDEpredict$fit[whiA,]), lty=1, lwd=2)
matlines(forecastyears, t(CDEpredict$fit[whiA,]+2*CDEpredict$se.fit[whiA,]), lty=2)
matlines(forecastyears, t(CDEpredict$fit[whiA,]-2*CDEpredict$se.fit[whiA,]), lty=2)
...
# LM
lm.male <- lca(usa.90, series="male", adjust="e0", years=1950:max(usa$year))
...
plot(usa.90, "male", years=bms.male$year[1]:max(usa$year), ylim=c(-10,0),
     lty=2, main="")  #main="BMS method: observed (1979-2010) and forecast (2011-2030) rates"
lines(forecast.bms.male)

#BMS USING LCA
bms.male <- lca(usa.90, "male", adjust="dxt", chooseperiod=TRUE, minperiod = 30, breakmethod = "bms")
forecast.bms.male <- forecast(bms.male, h=20)
bms.male$year[1]
...
plot(smus1950.100, series="male", 
	ylim=c(-10,0), lty=2, main="")  #main ="HU method: observed (1950-2010) and forecast (2011-2030) rates"
...
fdm.male <- fdm(smus, series="male", method="classical", weight=TRUE, beta=0.1)
forecast.fdm.male <- forecast.fdm(fdm.male, h=20)
plot(forecast.fdm.male)
...
par(mfrow=c(1,2))
plot(usa.pr$product$y, ylab="Log of geometric mean death rate",
	 font.lab=2, lty=2, las=1,  ylim=c(-10,-1),  main="Product function")
lines(usa.pr.f$product)
plot(sex.ratio(smus1950.100), ylab="Sex ratio of rates: M/F", ylim=c(0.7,3.5),
	 lty=2, las=1, font.lab=2, main="Ratio function")
lines(sex.ratio(usa.pr.f))

par(mfrow=c(1,2))
plot(smus1950.100, series="male", lty=2, ylim=c(-11,-1), main="Males")
lines(usa.pr.f$male)   
plot(smus1950.100, series="female", lty=2, ylim=c(-11,-1), main="Females")
...
plot(e0.fcast.m, ylim=c(65,85), col="blue", fcol="blue", 
	ylab="Years", main ="")  #main="Product-Ratio method: coherent life expectancy forecasts"
par(new=TRUE)
...
com.mx <- cbind(smus1950.100$rate$male[1:nrow(usa.pr.f150$male$rate$male),],
                  usa.pr.f150$male$rate$male)
birthyear <- 1950
mx <- com.mx[1:nrow(com.mx),(birthyear-1950+1):ncol(com.mx)]
cohort.mx <- diag(mx)

plot(log(cohort.mx), main="Cohort death rates US males born in 1950", 
       ylab="Log death rate", xlab="Age",cex=0.7)

mat3d <- persp(seq(0,99,by=3),seq(1950,1950+209,by=5),
               log(com.mx[seq(1,100,by=3),seq(1,210,by=5)]), theta=-30,
               xlab="Age", ylab="Year", zlab="Log death rate", ticktype="detailed",
               col=grey(.93), shade=TRUE, cex=0.7)
