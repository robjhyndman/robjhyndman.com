...
plot(
  fitBIC$x,
  log(dnk.ex2$rate$female[, "1950"]),
  xlab = "Year",
  ylab = "Log death rate",
  main = "Denmark: female death rates 1950",
  col = "gray"
)
...
plot(
  fitBIC$x,
  log(dnk.ex2$rate$female[, "2000"]),
  xlab = "Year",
  ylab = "Log death rate",
  main = "Denmark: female death rates 2000",
  col = "gray"
)
...
legend(
  ...
  lwd = 2,
  col = c(4, 2),
  legend = c("Lee-Carter", "Lee-Miller")
)
...
legend(
  ...
  lty = c(1, 2),
  col = 2,
  legend = c("Hyndman-Ullah", "Lee-Carter(unadj)")
)
...
legend(
  ...
  lty = c(1, 2, 1, 2),
  col = c(2, 2, 4, 4),
  legend = c("FDM female", "PR female", "FDM male", "PR male")
)
...
plot(
  life.expectancy(jpn.sm, series = "female", age = 0) -
    life.expectancy(jpn.sm, series = "male", age = 0),
  main = "Life expectancy difference: F-M",
  ylab = "Years",
  xlab = "Year",
  xlim = c(min(jpn$year), max(jpn$year) + 22),
  ylim = c(0, 7)
)
lines(
  flife.expectancy(jpn.pr.f$female, age = 0)$mean -
    flife.expectancy(jpn.pr.f$male, age = 0)$mean,
  lwd = 2
)
lines(
  flife.expectancy(forecast.fdm.female, age = 0)$mean -
    flife.expectancy(forecast.fdm.male, age = 0)$mean,
  lty = 2,
  lwd = 2
)
legend(
